...
def extract_element(ancestor, selector, attribute = None)
    try:
        return ancestor.select(selector).pop(0).text.strip(0)
    except: return None

product_id = input("podaj adres produktu")
...
while(page_no):
    ...
    for review in reviews: 
        ...
        author = extract_element(review,"span.user-post__author-name").pop(0).text.strip()
        try:
    ...
    f = open("reviews/3"+product_id+json", "w", encoding="UTF-8" )
